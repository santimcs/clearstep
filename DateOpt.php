<?php
  function selectDistinctDate ($connection, $tableName, $attributeName,
                           $pulldownName, $defaultValue)
  {
     ...
     $distinctQuery = "SELECT DISTINCT {$attributeName} FROM
                       {$tableName} ORDER BY {$attributeName} DESC LIMIT 60";

     // Run the distinctQuery on the databaseName
     if (!($result_date = @ mysql_query ($distinctQuery, $connection)))
        showerror( );
